...
#Mostrar imagenes
    ...
    public function actualizarSlideController($datos) {
        gestorSlideModels::actulizarSlideModel($datos, "slide");
        $respuesta = gestorSlideModels::selectActulizacionSlideModel($datos, "slide");
        echo json_encode($respuesta);
    }

    public function actualizarOrdenController($datos) {
        gestorSlideModels::actulizarOrdenModel($datos, "slide");
        $respuesta = gestorSlideModels::selectOrdenModel($datos, "slide");
        foreach ($respuesta as $row => $item) {
            echo '<li id="item' . $item["id"] . '">
     <span class="fa fa-pencil editarSlide" style="background:blue"></span>
     <img src="' . substr($item["ruta"], 6) . '" style="float:left; margin-bottom:10px" width="80%">
